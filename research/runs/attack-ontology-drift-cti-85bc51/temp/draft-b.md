...
Finally, where ATT&CK lives in practice. The Navigator version field is optional and silently defaults to the current release [15]; six of eight sample layers in MITRE's own Navigator repository carry no version [18]; the most widely used layer-generation tool omits the field by default, stamping its own build constant instead [79]. The declared version is the field the ecosystem most reliably leaves empty.
...
**Coverage claims.** The strongest empirical work shows ATT&CK coverage is not comparable across detection products, because vendors labelling the same behaviour choose different techniques within one pinned release [74]. MITRE's evaluation programme refuses to emit a single coverage score [46]; independent re-analysis argues per-technique tallies are the wrong unit [62]. Practitioners are blunter: a 100% claim is a red flag [80], heatmaps count rules rather than coverage under incompatible denominators [84], and vendor-side commentary enumerates the padding techniques [82]. Mitigation coverage has a ceiling set by the control catalogue [58]; Summiting the Pyramid replaces techniques with implementations as the denominator [64]; the ad hoc construction of layers over a versioned data-source mapping is documented from practice [81]. All of this establishes cross-product incomparability at a fixed version. It does not establish cross-version incomparability at a fixed product, which is Section 7.4.

**Benchmarks, extraction and attribution.** CTIBench introduced a multi-task benchmark including ATT&CK technique extraction [30]; successors grow the question set under a stable identifier [29], move to live-API evaluation to counter staleness [13], and expand the label space substantially [6]. Sequence-level benchmarks add tactic ordering as a drift surface [17]; large instruction corpora span two decades of reports without declaring a release [61]; multi-label work now declares existing annotation sets non-gold [52], a fork verifies labels against official descriptions [73], and synthesis work targets the long tail [65]. Answer-key decay in static benchmarks is quantified outside security [75]. On the systems side, TTPDrill remains the pre-sub-technique baseline [71]; TTPHunter and TTPXHunter expand the class count without declaring a release [72]; rcATT's released label space is a 2019-era flat vocabulary [44, 8]; CAPTAIN represents current TTP-sequence attribution, also undeclared [23]; an APT attribution survey organizes the field by artefact type rather than ontology version [12]; AttacKG works against an ontology frozen as a dated HTML scrape [2]. An audit of six TTP systems finds five different ATT&CK ontologies and two declared versions [3], and one remapping dataset declares a target release while carrying identifiers revoked five releases earlier [10]. Two results bound what TTP attribution can achieve at all: two-thirds of ATT&CK groups have no group-specific behaviour [59], and LLM agents reproduce documented APT profiles at 55-80% precision [66]. Multi-report evaluation finds many extractor errors are between same-tactic, description-overlapping techniques [20] — a rival explanation Section 7.2 must separate from drift, as must vendor fragmentation generally [68]. Telemetry-grounded evaluation is proposed as the structurally drift-resistant alternative [28]. What none of this work does is declare the ATT&CK release its labels encode, which Section 9 measures.
...
**O2. Deprecation.** $\ell \in \mathcal{L}_V \setminus \mathcal{L}_W$ with no successor; the concept is withdrawn. ATT&CK marks these and retains the object [1]. A pipeline filtering retired objects drops the label; one that does not silently carries a dead term.

**O3. Revocation with a typed successor.** $\ell \in \mathcal{L}_V \setminus \mathcal{L}_W$ with a `revoked-by` edge to a live $\ell'$. This is the well-handled case and the one the ecosystem's defence rests on: in our corpus every revoked Enterprise identifier resolves (Table 3), and the relation is a total function into the live label space.
...
The distribution matters more than the aggregate. The restructuring transition is 0.750 bookkeeping — but so is the most recent transition at 0.470, and two ordinary mid-decade transitions run at 0.380 and 0.340. Bookkeeping bursts are not confined to restructuring events. An analyst measuring how fast knowledge of an actor is growing from release-to-release edge counts attributes roughly a third of observed growth to intelligence that was never acquired — the same distinction between vocabulary churn and instance-data usage drawn explicitly in the knowledge-graph literature [37].
...
**The protocol must return a residual, not a set.** The current Enterprise crosswalk has 149 revocation edges, of which eight targets are many-to-one, absorbing twenty predecessors; the largest absorbs five. A set-valued protocol cannot distinguish a clean ten-identifier migration from one where three identifiers merged and two were dropped. This is not hypothetical: TRAM's bootstrap label set goes from 537 to 503 distinct live classes under normalization, with 0.1475 of its 25,770 instances landing in a merged class; rcATT goes from 215 to 199 with 0.0937 of its 6,235 instances merged. A benchmark whose class count silently collapses by 34 is not the same benchmark. We therefore redefine the return type as a ledger — kept, merged, dropped, demoted — and treat an unreported residual as a reporting failure.
