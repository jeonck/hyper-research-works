...
Make it concrete. Suppose you published a TTP-based attribution system in 2019 and reported that it beat the prior state of the art on a labelled corpus. A successor published in 2023 reports a higher number on the same corpus under the same metric. Between those dates the label space in which both express their predictions lost roughly half its identifiers in one release, gained a level of abstraction that did not previously exist, and rewrote three-quarters of the descriptions of the terms that survived. Neither paper declares which ATT&CK release it used; neither could, in practice, because no venue asks. That is not a weak comparison. It is a comparison between two measurement instruments whose readings are reported in the same units by convention alone.

Security machine learning has an unusually mature literature on this class of error. TESSERACT established that malware classifiers evaluated without temporal and spatial constraints report accuracies that do not survive deployment, and named the controls [67]; Arp et al. formalized ten recurring methodological pitfalls and showed much published work commits several [35]; CADE detects and explains drifting samples [22]; conformal evaluation supplies calibrated rejection [70]; dataset-shift taxonomies separate covariate shift from prior-probability shift from real concept drift [33]; malware-specific drift adaptation is its own subfield [50].

Every one of those instruments operates on a fixed label space. Dataset-shift taxonomy holds the class set constant [33]; TESSERACT's constraints govern sample timestamps, not class meanings [67]; conformal rejection calibrates against a known class set [70]; and the pitfall catalogue has no entry for a label space that changes between the training and evaluation corpora [35], because at the time no security label space was expected to. Where the class set does change, class-incremental learning addresses only the additive quadrant [24], and only recent work on evolving class ontologies treats relabelling as first-class [43]. Within security, one line frames ATT&CK version updates as concept drift and proposes incremental learning as the remedy [42] — closer than anything else, and still treating the ontology as a source of distributional disturbance rather than as the coordinate system the measurement is expressed in.

ATT&CK is that coordinate system. It is the label space of TTP extraction [71, 72], threat-actor attribution [23, 12], LLM CTI benchmarks [30, 29, 13], detection engineering and coverage reporting [74, 46], and the mitigation mappings that justify security spend [58, 64]. It is also, by its own design documentation, non-stationary: inclusion criteria evolve with observed adversary behaviour, and the abstraction level at which a behaviour is described is an acknowledged design decision rather than a fact about the world [14]. A practitioner defence argues the same from the other side — ATT&CK is an ontology rather than a taxonomy, and ontologies evolve [77]. We agree with both. That is the problem, not the answer.

This paper makes four claims. **First, ontology drift is a distinct, nameable threat to validity belonging in the same catalogue as temporal and sampling bias** (Section 4): concept drift changes the conditional distribution over a fixed class set, ontology drift changes the class set itself together with a partial, typed correspondence across releases, and a system perfectly robust to the former can still report an incomparable number because the incomparability is in the units. **Second, the threat is measurable** (Section 6), yielding a two-clock model — a slow episodic referential clock and a fast continuous intensional clock. The catastrophic identifier numbers the literature already cites [63] are one event, and a paper that headlines them deserves the rebuttal that the event is six years old; the numbers that survive are the semantic ones. **Third, drift changes conclusions, not scores** — the template comes from outside security, where Tomczak et al. froze a biological pipeline, swept the Gene Ontology version, and showed the biological conclusions moved [69]. **Fourth, the repair is partial and the unrepairable part is the invisible part:** identifier arithmetic repairs identifiers and says nothing about whether two identifiers mean the same thing.

We concede two things loudly, because the argument does not survive without them. MITRE's versioning apparatus is better than most scientific vocabularies: immutable per-release bundles, typed revocation edges with retained tombstones, a published change-computation tool, a version-stamped layer format [1, 7, 15, 34]; for Enterprise techniques the identifier accounting is complete. And the synchronic labelling noise floor is large: detection products assign different ATT&CK labels to the same behaviour within a pinned release [74], and most ATT&CK groups have no technique unique to them [59], so TTP attribution is weakly identifiable before drift enters. We do not claim drift dominates that noise. We claim it is separable by construction, we verify that with a factorial rather than asserting it, and we report the resulting bounds as bounds.

## 2. Background: ATT&CK as a Versioned Ontology

ATT&CK is published as immutable per-release STIX bundles at stable paths, and its usage contract recommends the static-copy workflow explicitly, because a downloaded copy does not change underneath an automated pipeline [1]. Pinning is not a community workaround; it is the documented first-class path, and any argument opening with an accusation of negligence is refuted by that one document.

The retirement contract is likewise normative. Objects no longer useful to track are *deprecated*; objects replaced by another object are *revoked* and carry a typed `revoked-by` relationship to the replacement; both are retained in every bundle so dependent workflows do not break, and a resolver and a retired-object filter ship alongside [1]. MITRE also publishes `diffStix`, whose change taxonomy includes a `patches` class for objects changed while their version field stays the same, and which concedes that version discipline broke historically [7, 34, 48]. ATT&CK Navigator defines a `versions` object carrying the content version a coverage layer was authored against [15] and documents a layer-upgrade flow [16]. CTID funds ATT&CK Sync to flag mappings affected by a release [5] — an institutional concession that staying in sync is an operational burden.

Three structural facts matter. **The March 2020 restructuring:** v7.0 introduced sub-techniques, a second level of abstraction. This was a re-cut, not an addition — techniques spanning several behaviours were revoked and replaced by parent-plus-children structures, and existing mappings were re-pointed to the most specific applicable child (Table 2). It is the most-cited instability in the literature [63] and also the best-handled event in the corpus. **The v19 tactic re-cut:** in April 2026 ATT&CK retired *Defense Evasion*, splitting its content between a renamed *Stealth* and a new *Defense Impairment* [78], with an industry narrative of rapid, unproblematic migration. The mechanism matters as much as the magnitude: the tactic identifier TA0005 was recycled in place, keeping its identifier while its name and referent changed, and no revocation edges exist on tactic objects at all, so a pipeline joining on TA0005 across that boundary compares two concepts with nothing to follow. **The 1:1 revocation relation:** `revoked-by` is total over revoked Enterprise techniques and never one-to-many — a stronger property than usually credited, and the source of a specific defect. A split cannot be expressed by a function into a single successor, so MITRE encodes splits as many-to-one merges onto the narrowest survivor; eight merge targets in the current bundle absorb twenty predecessors, the largest absorbing five. The crosswalk is mechanically total and semantically lossy, and the loss is unannounced — a consequence of the relation's arity, not an oversight in its population.

Finally, the declared version is the field the ecosystem most reliably leaves empty: the Navigator field is optional and silently defaults to the current release [15], six of eight sample layers in MITRE's own repository carry no version [18], and the most widely used layer-generation tool omits it by default [79].

## 3. Related Work

**CTI quality and ATT&CK as an object of study.** Feed evaluations find low overlap and limited timeliness [41, 45]; frameworks enumerate quality dimensions and propose instruments [60, 76]; automated and community schemes score feeds against weighted criteria [27, 38]; interview work documents quality assurance inside CTI teams [40]; a measurement-based survey reaches the same enumeration [11]. The recurring dimensions are accuracy, completeness, timeliness, relevance, provenance and interoperability, and none is defined over the *reference vocabulary*. We searched specifically for one and did not find it; that is a bounded absence, not a proof. A systematization of ATT&CK established that its results are frequently non-comparable and flagged temporal instability as open [63], and a subsequent survey set the bar a contribution must clear [47]. Both treat version instability as a known hazard; neither quantifies its effect on a downstream conclusion, which is the gap this paper occupies.

**Coverage claims.** The strongest empirical work shows ATT&CK coverage is not comparable across detection products, because vendors labelling the same behaviour choose different techniques within one pinned release [74]. MITRE's evaluation programme refuses to emit a single coverage score [46] and independent re-analysis argues per-technique tallies are the wrong unit [62]; practitioners add that a 100% claim is a red flag [80], that heatmaps count rules under incompatible denominators [84], and that a coverage number is routinely padded [82]. Mitigation coverage has a ceiling set by the control catalogue [58], Summiting the Pyramid replaces techniques with implementations as the denominator [64], and layer construction over a versioned data-source mapping is ad hoc in practice [81]. All of this establishes cross-product incomparability at a fixed version, not cross-version incomparability at a fixed product — which is Section 7.4.

**Benchmarks, extraction and attribution.** CTIBench introduced a multi-task benchmark including ATT&CK technique extraction [30]; successors grow the question set under a stable identifier [29], move to live-API evaluation to counter staleness [13], and expand the label space substantially [6]. Sequence-level benchmarks add tactic ordering as a drift surface [17]; large instruction corpora span two decades of reports without declaring a release [61]; multi-label work now declares existing annotation sets non-gold [52], a fork verifies labels against official descriptions [73], synthesis work targets the long tail [65], and answer-key decay is quantified outside security [75]. On the systems side, TTPDrill remains the pre-sub-technique baseline [71], TTPHunter and TTPXHunter expand the class count without declaring a release [72], rcATT's label space is a 2019-era flat vocabulary [44, 8], CAPTAIN represents current TTP-sequence attribution [23], an attribution survey organizes the field by artefact type rather than ontology version [12], and AttacKG works against an ontology frozen as a dated HTML scrape [2]. An audit of six TTP systems finds five different ATT&CK ontologies and two declared versions [3], and one remapping dataset declares a target release while carrying identifiers revoked five releases earlier [10]. Two results bound what TTP attribution can achieve at all: two-thirds of ATT&CK groups have no group-specific behaviour [59], and LLM agents reproduce documented APT profiles at 55-80% precision [66]. Multi-report evaluation finds many extractor errors are between same-tactic, description-overlapping techniques [20] — a rival explanation Section 7.2 must separate from drift, as must vendor fragmentation generally [68]; telemetry-grounded evaluation is the proposed drift-resistant alternative [28]. What none of this work does is declare the release its labels encode, which Section 9 measures.

**Concept drift, and ontology engineering elsewhere.** The formal machinery of drift is mature [33] and its security instantiation well developed [22, 67, 70, 50]; open and unstable class sets have been studied in malware family labelling [51], and label normalization across inconsistent vendor vocabularies has prior art in AVClass [19]. Class-incremental learning covers additive change [24], continual learning with evolving class ontologies covers relabelling [43], and IncreTTP is the closest security work [42]. This literature stops where the label space starts moving in non-additive ways, and synthesising that boundary motivates our formalization [21]. Everything ATT&CK lacks exists in ontology engineering: OBO Foundry mandates identifier uniqueness, non-reuse and persistence [53], version IRIs and release immutability [55], and referent stability with exact-versus-inexact successors [54]; OWL supplies `versionIRI`, `priorVersion` and `backwardCompatibleWith` [57]; change management is the recognized core task of versioning [56]; COnto-Diff types complex change operations as evolution mappings [26]; vocabulary churn versus instance-data usage has been studied as two time series in knowledge graphs [37]; and Tomczak et al. showed Gene Ontology evolution changes the interpretation of biological experiments [69]. Within security, vocabulary instability is documented for CVE lifecycle states [31], CVE-CWE-CPE mappings [32], CVSS fragmentation [39] and NVD data quality [25].
...
Let $V$ index the published releases of an ATT&CK domain. A release determines a label space $\mathcal{L}_V$, the technique identifiers live in it, together with an *intension* map $\iota_V : \mathcal{L}_V \to \mathcal{D}$ assigning each identifier the description, tactic assignment, platform set and detection guidance that define what it denotes. A CTI analytic is a function $f : \mathcal{X} \to 2^{\mathcal{L}_V}$ from evidence to a set of labels. A CTI artefact is a pair $(A, V)$ with $A \subseteq \mathcal{L}_V$. In practice $V$ is almost never recorded (Table 12), so what circulates is $A$ alone.

Two artefacts are *comparable* when expressed in the same label space under the same intension map, and two results are comparable when the analytics producing them are. That is not a strong requirement. It is the ordinary requirement that measurements reported in the same units be reported in the same units.

### 4.2 Why this is not concept drift

Concept drift is defined over a fixed label space: the standard taxonomy decomposes change in $P_t(X, Y)$ into covariate shift, prior-probability shift and real concept drift, all with $\mathcal{Y}$ constant [33]. The security instruments built on it inherit the assumption [67, 22, 70, 35]. Class-incremental learning relaxes it in one direction only — classes are added, previously learned classes retain their meaning [24] — which covers ATT&CK's additions and nothing else. The only close formalism is continual learning with evolving class ontologies, which models old data relabelled under a refined class structure [43], and even that assumes the refinement is known, typed and complete. In ATT&CK it is partially typed, occasionally lossy, and for the component that matters most, entirely unrecorded.

The distinction is this. Concept drift moves the data under a fixed ruler; ontology drift moves the ruler. A model perfectly robust to concept drift — retrained continuously, conformally calibrated, deployed under a rigorous time-aware split — still produces an incomparable number if its label space changed and it did not say so. Robustness is not the remedy, because the failure is not in the predictions but in the interpretation of the outputs.
...
**O1. Addition.** $\ell \in \mathcal{L}_W \setminus \mathcal{L}_V$ with no predecessor — the only operator benign for backward comparison and the only one class-incremental learning handles [24]. It is not benign forward: a growing coverage denominator makes a frozen capability's percentage fall with no change in the capability, the mechanism behind Table 8.

**O2. Deprecation.** $\ell \in \mathcal{L}_V \setminus \mathcal{L}_W$ with no successor; the concept is withdrawn, marked, and the object retained [1]. A pipeline filtering retired objects drops the label; one that does not silently carries a dead term.

**O3. Revocation with a typed successor.** $\ell \in \mathcal{L}_V \setminus \mathcal{L}_W$ with a `revoked-by` edge to a live $\ell'$ — the well-handled case, and the one the ecosystem's defence rests on: in our corpus every revoked Enterprise identifier resolves (Table 3), and the relation is a total function into the live label space.
...
**O5. Cross-layer reassignment.** A concept moves between levels. In the current Enterprise bundle 119 of 149 revocation edges demote a top-level technique onto a sub-technique and seven promote in the opposite direction, with no abstraction-level annotation, so a consumer cannot detect that the granularity of its assertion changed. The v19 promotion of a technique to a *tactic* is not representable at all, because a revocation edge cannot point a technique at a tactic. Instance-level tactic reassignment is also unsignalled: 198 identifier-stable techniques changed tactic in one transition (Table 2), and tactic objects carry no revocation edges.
...
Drift does not run at one rate. Let $\sigma_V(t)$ be the fraction of a cohort live at $V$ still live at $t$, and $\tau_V(t)$ the fraction whose intension has substantively changed. The referential clock, characterized by $\sigma$, is episodic: long quiet periods punctuated by restructuring events that re-identify a large share of the catalogue at once. The intensional clock, characterized by $\tau$, is continuous and has barely varied since 2020. The consequence is that the two clocks give opposite answers to the practitioner's question, *how long is my pinned label set good for?* — infinite on identifiers for any post-2020 Enterprise cohort, and roughly eighteen months to first 10% substantive staleness. A risk model reporting only one is wrong in a predictable direction, and the direction depends on which one it reports.

### 4.6 Ontology drift as a threat to validity

We can now place the phenomenon in the experimental-methodology tradition. Temporal bias is the failure to respect the arrow of time in partitioning, controlled by the time-aware split; spatial bias is the failure to respect deployment base rates, controlled by a realistic test-set class ratio [67]. Sampling bias, label inaccuracy, data snooping and inappropriate baselines each have a recognized name and control [35]. Ontology drift is the failure to hold the label space fixed across the comparisons a paper makes, and it damages validity in four distinguishable ways.

*Construct validity.* After O6 the same label denotes a different behaviour, so the construct measured is not the construct the labels name: a system trained on a technique as defined in 2020 and evaluated against gold labels assigned under the 2026 definition measures agreement between two constructs. *Internal validity.* After O4 or O5 a label set's cardinality and granularity change under normalization — a profile containing four documented behaviours can, after mechanical migration, contain three, so measured breadth falls with no change in the underlying intelligence and any analysis treating profile size as a feature inherits the artefact. *External validity.* After O1 the denominator of any coverage or recall claim grows: an unchanged capability reports a falling number, and if the denominator shrinks it reports a rising one. Neither movement is about the world. *Conclusion validity.* This is the one that matters for a journal. If the effect were confined to scores the appropriate response would be an error bar; Section 7.3 shows it reaches the named actor, the top-ranked mitigation and the pass/fail verdict on a coverage target.

The control is not a better algorithm. It is declaration plus projection: state the release, the domain and the bundle hash, and project both sides of any cross-time comparison onto one reference release with a published residual. Section 10.2 gives the contract; Sections 8 and 11 state how far it goes and where it stops.

### 4.7 What a valid measurement of drift requires

A study of drift is unusually exposed to its own subject, so we commit in advance to four requirements, each audited in Section 11. **R1. Fixed labelling process across conditions** — the identifying contrast must hold everything constant except the vocabulary, or it is confounded with intelligence content. **R2. A stated noise model** — holding noise at zero establishes separability and says nothing about additivity; a study claiming additivity must vary the noise rate. **R3. A declared closure boundary** — if observation, profile, ground truth and migration map derive from one curator, the absolute levels are internal-consistency scores and must never be reported as task performance. **R4. A named falsifier.**
...
Seven measurement families support the argument: per-release churn (E1); identifier survival to v19.0 with recoverability through the transitive closure of the revocation graph (E2); intensional drift among identifier-stable techniques, by token Jaccard over descriptions (E3); decomposition of each newly added group-technique edge into genuine new intelligence, an edge to a new technique, a sub-technique refinement, or a revocation re-mapping (E4); the controlled attribution experiment (E5); the frozen-capability coverage experiment with a prevalence-weighted variant (E6); and label validity of four deployed CTI corpora dated against every release (E7). A further analysis (E10) re-runs attribution and mitigation ranking at the level of conclusions. Auxiliary analyses cover version-metadata reliability, the v19 revocation wave, the tactic layer, noise injection, stratification and corpus integrity.

**The attribution design.** Fix a modern release $W$ = v19.0 and a legacy release $V$. Per trial, sample a group documented at $W$ and draw $k = 10$ observed techniques from its modern profile, then back-project each into $\mathcal{L}_V$: pre-revocation identifier where one exists, surviving ancestor where the sub-technique did not yet exist, drop where no $V$-era ancestor exists. This yields a $V$-vocabulary observation of a $W$-era intrusion. Four conditions score it. **Back-projected** (labelled *contemporaneous* in our released code and tables) scores against $V$-vocabulary profiles — self-consistent, no mismatch. **Naive** scores directly against $W$-vocabulary profiles — the analyst who ignores drift. **ATT&CK-Norm** applies the Section 8 protocol first. **Oracle** scores the original $W$-vocabulary observation against $W$ profiles. The *drift penalty* is back-projected minus naive accuracy; *recovery* is the fraction normalization returns. Every condition consumes the same intelligence content and the same draws, satisfying R1: the contrast cannot be explained by a difference in what was observed. Because all four draw labels from the same curator's data, inter-labeller noise is identically zero in all of them and cancels in the contrast; Section 7.2 varies it. Robustness is assessed across three scoring functions and two profile definitions (Table 7).

**Evidence discipline.** This paper's subject is the reliability of what artefacts declare about themselves, so we state our own tiers. **Tier A** covers our measurements over the bundles and the repository artefacts we read in full; these are reproduced by the released code. **Tier B** covers secondary literature reached in this environment only through search summaries, because the egress path blocks publisher sites and preprint servers; every Tier B claim is attributed as reported, and no verbatim quotation is asserted from a source we did not read in full. **Tier C** covers audited absences, stated with the bound of the search that found them and never upgraded to proofs. Section 11.6 reports what this cost.
...
Pre-restructuring cohorts lose most of their identifiers, with half-lives of 0.4 to 2.2 years that all land on the same release. Every cohort from v7.0 onward retains at least 0.970 and none reaches a half-life. The `Recoverable` column equals the `Revoked` column in every row: every revoked identifier in the corpus resolves to a live identifier through the published revocation graph. We make that concession in the results, not a footnote. On identifiers alone, and for Enterprise alone, the position that drift is solved bookkeeping is correct.

It is not correct for Enterprise alone. Mobile has a 2018 transition with an identifier Jaccard of 0.000 in which 76 techniques vanish with no tombstone and no successor, none recoverable — a wholesale renumbering more destructive than the Enterprise restructuring and eighteen months earlier — and ICS is undergoing its own sub-technique restructuring now, six years after Enterprise. Counting transitions with identifier Jaccard below 0.80 across the three domains gives five events in eight years: a hazard of 0.106 per major transition, or roughly one restructuring somewhere in ATT&CK every 1.5 calendar years. The 2020 event is a draw from a live hazard, not a closed wound. Nor can a consumer time-hedge: across seventeen lagged release pairs the best leading indicator of the next transition's revocation rate reaches a Spearman correlation of +0.377 at a permutation p-value of 0.134, and techniques revoked at v7.0 had been *edited less* beforehand than those that survived. Restructuring is not forecastable from the public bundles — a negative result that forces the protocol of Section 10.2 to be standing rather than triggered.
...
Across all Enterprise transitions 5,516 group-technique edges were added, of which 2,442 belong to newly added groups. Of the 3,074 added for pre-existing groups, 1,752 are genuine new intelligence, 330 are edges to newly added techniques, 487 are sub-technique refinements of an assertion already present, and 505 are re-mappings forced by a revocation. The bookkeeping share is 992 of 3,074, or 0.323.

The distribution matters more than the aggregate. The restructuring transition is 0.750 bookkeeping — but so is the most recent transition at 0.470, and two ordinary mid-decade transitions run at 0.380 and 0.340, so bookkeeping bursts are not confined to restructuring events. An analyst measuring growth from release-to-release edge counts attributes roughly a third of it to intelligence that was never acquired — the distinction between vocabulary churn and instance-data usage drawn explicitly in the knowledge-graph literature [37].
...
The strongest published objection is not that our numbers are wrong but that they are irrelevant, because a much larger source of label variance already exists at a single pinned release. Virkud et al. show detection products describing the same behaviour carry different ATT&CK labels within one pinned release, often with no overlap [74]. Saha et al. show roughly a third of ATT&CK groups have any technique unique to them [59]. Multi-report evaluation adds that many extractor errors are between same-tactic, description-overlapping techniques [20], and vendor fragmentation has been characterized as an echo-chamber effect any drift claim must separate itself from [68]. If a labelling process has that much synchronic variance, why care about a 1.6-point diachronic effect?
...
None of this is invented here; all of it is standard ontology-engineering practice and would be a port rather than a research programme. A two-tier successor vocabulary distinguishing exact from inexact successors, as OBO Foundry Principle 19 specifies [54], would let a split be expressed as several inexact pointers instead of one misleading exact one, and a controlled obsolescence-reason vocabulary — superseded-by-split, merged, re-scoped, promoted-to-tactic, out-of-scope — would make drop and merge distinguishable, which today they are not. Referent stability at a stable identifier is the principle that renaming a tactic in place violates [53, 54]; prior-version and backward-compatibility links are standard OWL vocabulary [57] and ATT&CK has neither, so lineage is reconstructed by sorting release names. Publishing the evolution mapping as a first-class versioned artefact typed with complex change operations is exactly what COnto-Diff specifies [26]; `diffStix` already computes the underlying operations [34, 48], and typing them is what would make the split of Table 5 computable rather than asserted. Making the Navigator version field mandatory [15] would close the largest adoption gap we measured.

### 10.4 The missing quality dimension, and the bar for a contribution

CTI quality frameworks enumerate accuracy, completeness, timeliness, relevance, provenance and interoperability [60, 76, 27], and practice studies describe what teams do [40]. Across the sources we could reach, none defines a dimension over the reference vocabulary. We state that as an audited absence over a bounded search and propose *vocabulary currency* as the missing dimension — operationalized by the four lines above and measurable, for any corpus, by Section 9. It is a quality property in the ordinary sense: a property of the artefact, checkable without access to the producer, degrading predictably at a rate this paper estimates.

The query also asks what an SCI-level contribution here must demonstrate. Naming drift is not enough: the SoK already flags temporal instability [63] and MITRE already publishes the diff [34]. Counting churn is not enough either; anyone can run `diffStix`. The bar, set by the Gene Ontology precedent [69], has four parts. A contribution must (i) formalize the phenomenon so it is distinguished from concept drift rather than subsumed by it, (ii) measure it longitudinally over the full published release history rather than at selected boundaries, (iii) demonstrate that it changes *conclusions* under a design holding the labelling process fixed, so the effect is not a re-description of known synchronic noise [74, 59], and (iv) state honestly what the proposed repair cannot do. A paper that does (i) through (iii) and skips (iv) has produced a tool paper with an inflated claim. That is why Section 11 is long.
...
The prevalence-weighted coverage analysis weights techniques by documented `uses` edges, which are not sightings. A `uses` edge counts how many groups, malware families and tools MITRE has documented as employing a technique: a cumulative stock over cited CTI reports, monotone in time because a 2019 edge never decays, and biased toward behaviours easy to narrate. Defender telemetry measures a flow over a window and is materially more concentrated [49]. Worse for us, the weight is computed from the same release whose churn is measured, so a revoked technique carries its weight out of the corpus with it; the weighted artefact is not independent evidence in the way an external telemetry distribution would be. The correct scope statement is therefore narrow and we hold to it: the long-tail objection is refuted for the fraction of the *written CTI corpus* affected by drift, and untested for the fraction of the *alert stream* affected, because the telemetry that would test it on its own data is not reachable here. A reader who believes the alert stream is far more concentrated than the documentation stock — and the published bias work suggests it is [49] — should discount our prevalence-weighted result accordingly. It would not change the unweighted, attribution or label-validity results.
...
The bookkeeping classification in Table 5 is rule-based and conservative: an edge is classified as bookkeeping only when mechanical evidence is present, so an edge that is substantively a re-description but carries no crosswalk evidence counts as genuine new intelligence. The 0.323 aggregate is a lower bound under our definitions, of a quantity whose definition is itself contestable. The identifier half-life in Table 3 is computed on the major-release grid, so the six pre-restructuring half-lives are measurements of the same event at different distances from it; reading them as six independent observations of a decay process would be wrong, and we report them as one event seen six times.
...
The four corpora in Table 9 are not a random sample: they were selected because their label files are public and machine-readable, which biases toward open research artefacts and away from commercial corpora that may be better or worse maintained, and the adoption evidence is similarly drawn from what could be cloned. The direction of that bias is unknown to us.

The attribution task is top-1 identification within a candidate universe of documented ATT&CK groups at $k = 10$ observed techniques. Real attribution uses infrastructure, malware lineage, targeting and timing alongside TTPs; a purely TTP-based top-1 task is a deliberately narrow instrument chosen because it isolates the vocabulary, and results should not be read as statements about attribution practice. Work suggesting LLM agents reproduce documented APT profiles at high precision [66] is a further reason to treat profile-matching accuracy as a property of the documentation rather than of the adversary.

Finally, these numbers have a shelf life: Table 4 measures every cohort to a fixed endpoint that moves with every release, and under the two-clock model the identifier columns will be stable while the semantic columns grow. That this paper's own results require a declared reference release to be comparable to a future replication is confirmation of its thesis rather than an embarrassment, and the released artefacts declare it.
...
We are explicit about what we did not show. Not that drift dominates the synchronic labelling noise floor — our design deliberately does not measure that floor; we showed separability by construction and verified it under injected noise, which also told us the headline pre-restructuring penalties are upper bounds. Not that identifier normalization restores comparability — it recovers about half the penalty on legacy artefacts, under a point in the modern regime, and nothing at all about meaning. And we did not escape our own subject: our evidence pipeline suffered a provenance failure in ten of 112 notes, found, quarantined and re-collected, which stands as this paper's own demonstration that undeclared provenance is not a hypothetical risk.
