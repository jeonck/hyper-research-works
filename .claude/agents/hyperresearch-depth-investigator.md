...
1. **Start with the vault.** Before fetching anything new, read the notes
   ...
   `/home/user/hyper-research-works/.venv/bin/hyperresearch note show <id1> <id2> <id3> --json`
...
3. **Fetch new sources via the fetcher subagent.** Do NOT call
   `/home/user/hyper-research-works/.venv/bin/hyperresearch fetch` directly. Delegate to `hyperresearch-fetcher` via the
...
5. **Read the fetched sources.** Use `/home/user/hyper-research-works/.venv/bin/hyperresearch note show <id> -j`. Quote
...
6. **Write ONE interim report note.** This is your single deliverable.
   ...
   **BEFORE calling `note new`**, check if an interim note for this locus
   ...
   ```bash
   /home/user/hyper-research-works/.venv/bin/hyperresearch note list --tag locus-<locus-name> --type interim --all --json
   ```
...
```bash
/home/user/hyper-research-works/.venv/bin/hyperresearch note new "Interim report — <locus name>" \
  --tag <corpus_tag> \
