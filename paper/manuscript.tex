...
- a trained classifier carried across eras with the vocabulary component separated from the data-drift component, two real systems on three real gold sets with per-item verdicts, named public coverage layers re-measured, and language-model extractors whose emitted identifiers are classified as live, stale or fabricated against every release (Sections 7.8 to 7.10)
...
\subsection{3.1 The closest neighbours, read in full}
...
\textbf{Table 26.} The closest neighbours on five questions. Every cell is from the full text except the IncreTTP row, which is from the abstract.
...
Six operators carry \texttt{(L\_V, ι\_V)} to \texttt{(L\_W, ι\_W)}.

- \textbf{O1 Addition}: a new identifier with no predecessor, the only operator class-incremental learning handles [24] and the only one benign for backward comparison; forward it is not benign, because a growing denominator makes a frozen capability's percentage fall with no change in the capability (Table 8).
- \textbf{O2 Deprecation}: withdrawn with no successor, marked and retained [1].
- \textbf{O3 Revocation with a typed successor}: the well-handled case on which the ecosystem's defence rests, and in our corpus every revoked Enterprise identifier resolves to a live one (Table 3).
- \textbf{O4 Re-cut}: a concept split across successors, or several merged into one. Because \texttt{revoked-by} is a function and never one-to-many, a split is representable only as merges onto the narrowest survivor. It is not true that the relation cannot express the v19 re-cut; it can, as merges. The defect is annotation rather than expressiveness — the merge is unlabelled, so a consumer applying the crosswalk mechanically cannot distinguish an exact successor from a narrowing, which ontology engineering settled by splitting the successor relation into exact and inexact variants [54] and typing complex change operations [26, 56].
- \textbf{O5 Cross-layer reassignment}: a technique demoted onto a sub-technique, a sub-technique promoted, or a technique promoted to a tactic; the crosswalk carries no abstraction-level annotation, and a promotion to the tactic layer is not representable at all, since \texttt{revoked-by} cannot point a technique at a tactic and no tactic object in the corpus carries a revocation edge.
- \textbf{O6 Intensional rewriting}: the identifier survives and what it denotes moves, invisible to every identifier-based mechanism in the ecosystem.
...
\textbf{Table 2.} Per-release Enterprise churn across consecutive major releases. Rewrite and tactic columns count only techniques live in both releases; \texttt{J(ID)} is the identifier Jaccard; edges are group-to-technique \texttt{uses} edges. All eighteen transitions are in Figure 1 and the package.
...
\textbf{Table 3.} Identifier survival to v19.0 by source release. Recoverable counts revoked or absent identifiers whose transitive closure terminates at a live identifier. All nineteen cohorts are in Figure 2 and the package.
...
\textbf{Table 4.} Semantic drift among techniques live at both the source release and v19.0. Token Jaccard is over lowercased alphanumeric token sets of the description; a substantial rewrite is below 0.8. All eighteen cohorts are in Figure 3 and the package.
...
\textbf{Table 13.} Text change versus version increment for techniques live in both releases of a consecutive major pair, Enterprise. Full rows in the package.
...
\textbf{Table 27.} The paper's rule, J < 0.8, read against the pilot annotation as a detector of substantive change (level 2 or 3). Population-weighted figures re-weight each item by its Jaccard bin's share of the 1,358 edited pairs. The agreement subset is the 139 items both annotators binarised the same way.
...
\textbf{Table 5.} Knowledge-growth decomposition, Enterprise. Edges for groups newly added to ATT\&CK are excluded from the denominator; bookkeeping share is refinement plus re-mapping over all new edges for pre-existing groups. Per-transition rows are in Figure 4 and the package.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{fig10_cwe_capec.pdf}
\caption{The two clocks in three MITRE vocabularies: identifier-set Jaccard across consecutive releases, and edited and substantially rewritten shares by cohort age, for ATT\&CK Enterprise, CWE and CAPEC.}
\label{fig:10}
\end{figure}


\textbf{Figure 10.} The two clocks in three MITRE vocabularies: identifier-set Jaccard across consecutive releases (left) and the share of identifier-stable entries with an edited description, and with a substantial rewrite, by cohort age (right).

\textbf{Table 24.} ATT\&CK Enterprise against CWE and CAPEC on the same instrument. Horizon: the analysed cohort nearest five years before each vocabulary's newest release, measured at that release over identifier-stable, non-deprecated entries. Structural reassignment is a tactic-set change in ATT\&CK and an abstraction-level or parent change in CWE and CAPEC.
...
\textbf{Table 6.} Controlled attribution, k = 10, analysis release v19.0, 500 trials per row, paired bootstrap intervals. Back-projected is the self-consistent legacy system; naive is the same artefact read by the modern base; recovered is the share of the penalty normalization returns. All eighteen vocabularies are in Figure 5.
...
\textbf{Table 21.} Multiple-comparison control over the 54 attribution rows. Exact paired sign test per row on the per-trial top-1 differences; Holm and Benjamini-Hochberg adjustment across all rows of each contrast; survivors at α = 0.05. Per-row p-values are in the package.
...
\textbf{Table 7.} Scorer and profile robustness, k = 10. Software-mediated: whether techniques reached only through a group's software are included in its profile. Penalty and gain in points of top-1 accuracy.
...
\textbf{Table 14.} Noise factorial, k = 10. Labeller noise is injected before back-projection at rate ρ, so it flows into every condition. Self-consistent is the back-projected control.
...
\textbf{Table 16.} Archival control, k = 10. The archival artefact is a draw from the group's profile as actually published at the source release, not a back-projection of its modern profile.
...
\textbf{Table 15.} Specificity stratification, k = 10. A group has a unique technique if at least one identifier in its profile appears in no other candidate's profile at the source release. Penalties are top-1 differences with paired bootstrap 95\% intervals.
...
\subsection{7.8 A trained classifier, with vocabulary drift separated from data drift}
...
Four scorings share the same predictions. \emph{Naive} scores the model's identifiers against the gold verbatim. \emph{Normalized} projects both sides onto v19.2 with ATT\&CK-Norm. \emph{Vocabulary-matched} back-projects the modern side into the older vocabulary with the map of Section 7.2, so the same predictions meet a gold set that could have been written in the model's own label space; the difference between it and naive is the cost of the vocabulary alone. The \emph{ceiling} is a five-fold cross-fitted model trained on the test corpus itself, in the matched vocabulary, so it shares both the label space and the text distribution; the difference between it and vocabulary-matched is what the data shift cost. Paired bootstrap over test documents gives the intervals and Holm-corrected p-values (Table 18).

\textbf{Table 18.} A v6.3-era classifier evaluated on the v13.0 TRAM gold, and the reverse, with the total cross-era loss decomposed. Micro-F1 over label instances; sample-F1 is the per-document mean. Deltas are paired bootstrap over documents, 2,000 resamples, Holm-corrected within each pair. Full rows, macro-F1, precision and recall, the CTIBench pair and the projection ledgers are in the package.
...
\subsection{7.9 Two real systems, three real gold sets, and named published layers}
...
\subsection{7.10 LLM extractors: stale, fabricated, or live?}

Deployed extractors are increasingly language models rather than trained classifiers [61, 72, 73], and a language model changes the question. Its label space is not a file it was trained on but whatever ATT\&CK looked like in its pre-training data, so when it is asked for a technique identifier it answers from a release nobody declared, and an identifier that is wrong today may be wrong because the catalogue moved or because the model invented it. The two are different failures with different remedies, a stale identifier being an alias the revocation graph resolves in the manner of malware-label normalization [19] and a fabricated one being noise, and the release database can tell them apart. We prompt two models run locally with pinned decoding, gemma3:4b and llama3.2:3b, temperature zero, fixed seed, model digests recorded, the hygiene the pitfall catalogues ask for [35, 67], to extract Enterprise technique identifiers from 150 TRAM sentences [3], the 47 CTIBench Enterprise items [30] and 50 rcATT reports [8], under four release-naming conditions: no release named, which is the deployed default; the current release, v19.2; the gold set's own release; and v6.3, the vocabulary before sub-techniques existed. Every emitted identifier is then classified against all forty-one Enterprise releases as \textbf{live} at v19.2, \textbf{stale} (live in some earlier release and revoked, deprecated or absent now, the only class ontology drift can produce), or \textbf{fabricated} (never an Enterprise identifier in any release), and the model's \textbf{home release} is the release at which the largest share of its distinct identifiers is live, the provenance-interval method of Section 9 applied to a model instead of a corpus (Table 28).

\textbf{Table 28.} Two local language models as technique extractors, 247 items, four prompt conditions. Shares are over emitted identifier instances. Stale identifiers were all resolvable through the revocation graph except one instance. Prompt-sensitivity rows and the per-gold-set scoring are in the package.

\begin{center}\footnotesize
\begin{tabular}{llllllll}
\toprule
Model & Prompt condition & Emitted (distinct) & Live at v19.2 & Stale & Fabricated & Home release (share live there) & Emitted share live at v6.3, rcATT items \\
\midrule
gemma3:4b & unspecified & 908 (105) & 0.831 & 0.109 & 0.059 & v10.1 (0.64) & 0.185 \\
gemma3:4b & latest, v19.2 & 901 (118) & 0.820 & 0.120 & 0.060 & v18.1 (0.64) & 0.205 \\
gemma3:4b & gold set's release & 899 (109) & 0.819 & 0.122 & 0.059 & v18.1 (0.62) & 0.222 \\
gemma3:4b & v6.3 & 882 (95) & 0.840 & 0.138 & 0.022 & v18.1 (0.73) & 0.215 \\
llama3.2:3b & unspecified & 1193 (61) & 0.845 & 0.137 & 0.018 & v6.3 (0.64) & 0.798 \\
llama3.2:3b & latest, v19.2 & 1198 (64) & 0.826 & 0.128 & 0.046 & v6.3 (0.66) & 0.695 \\
llama3.2:3b & gold set's release & 1185 (58) & 0.833 & 0.149 & 0.019 & v6.3 (0.74) & 0.817 \\
llama3.2:3b & v6.3 & 1161 (45) & 0.841 & 0.158 & 0.002 & v6.3 (0.96) & 0.996 \\
\bottomrule
\end{tabular}
\end{center}

Four things are visible before any score is computed. First, stale exceeds fabricated. Between 0.11 and 0.16 of every identifier these models emit is a real ATT\&CK identifier that no longer denotes, against 0.00 to 0.06 invented outright, so for these models the larger source of invalid output is the catalogue having moved under them, not hallucination in the usual sense, and every stale identifier but one resolves through the revocation graph. The stale identifiers are the 2020 re-cut's casualties, T1028, T1073, T1077, T1085, T1089 and their kind, plus a few revoked sub-techniques such as T1053.001 and T1070.001, every one of them carrying a \texttt{revoked-by} edge under the published contract [1]. Second, the fabrications have a shape. gemma3's invented identifiers are almost all phantom sub-techniques, T1018.001, T1057.001, T1059.100, sub-technique numbering extrapolated onto parents that have none or beyond the children that exist, which is the model having learned the pattern of the 2020 re-cut, the instability both systematizations flag [47, 63], without its content. Third, naming a release in the prompt does not move the model's vocabulary. Across the four conditions gemma3's stale share moves from 0.109 to 0.138 and llama3.2's from 0.128 to 0.158, and asking either for v19.2 does not lower it; the only prompt with a visible effect is the request for v6.3, which cuts fabrication to 0.022 and 0.002 because a parent-only vocabulary gives the model fewer sub-technique slots to invent, and which llama3.2 satisfies almost perfectly, 0.996 of its rcATT output live at v6.3. A model answers from its parametric release, and a release named in a prompt is a request it cannot honour except downward. Fourth, the two models live in different releases. llama3.2's best-fit release is v6.3 under every condition, with 0.64 to 0.96 of its distinct identifiers live there: it is a pre-sub-technique extractor. gemma3's best fit is v10.1 or v18.1 at 0.62 to 0.73, and no release makes more than three-quarters of its identifiers simultaneously live, which is the mixed-vocabulary signature the TRAM bootstrap corpus shows in Section 9 [3, 52, 73], produced here by one model in one sitting.

Scored against gold the models are poor extractors and we do not pretend otherwise, far below the extraction line's own reported figures [20, 72]: micro-F1 is below 0.06 in every cell and llama3.2 scores 0.000 on the TRAM sentences under every prompt. What survives the low ceiling is the direction of the vocabulary effect. gemma3, a modern-vocabulary model, emits identifiers of which only 0.185 to 0.222 are live in v6.3, so a 2019 gold set cannot read its output; projecting both sides onto v19.2 raises its micro-F1 on the rcATT reports from 0.017 to 0.062 unspecified and from 0.030 to 0.081 under the v19.2 prompt, a sample-F1 gain of 0.041 to 0.050 with intervals excluding zero, and one report crosses the pass threshold that none did before. On the modern gold sets the gain is 0.001 to 0.006 and the passing count moves by one or two items. llama3.2, a legacy-vocabulary model, gains little from projection on any gold set because its stale identifiers are the parent techniques the crosswalk demotes onto sub-techniques, the O5 case and the inexact successor the ontology literature separates from an exact one [54], and demotion does not create a match the gold does not contain. Three paraphrases of the unspecified prompt on sixty TRAM sentences hold gemma3's stale share between 0.099 and 0.112 and its fabricated share between 0.036 and 0.083, so the classification is stable to wording while the fabrication rate is the more prompt-sensitive of the two, which is another reason to keep them apart.

The experiment applies the paper's own contract to itself. Its prompts, seeds, decoding parameters and the digest of each model weight file are in the package, because a language-model result denominated in ATT\&CK identifiers has two undeclared releases, the catalogue's, which the benchmark audits of Section 9 found hard-coded and unstated [4, 6], and the model's, and Table 28 shows the second can be read off the output. The limit is the models: 4-billion and 3-billion parameter weights run locally because this environment has no API access (Section 11). A frontier model will fabricate less and sit at a later home release; whether it is also less stale is the question this design answers for any model that exposes a prompt, and it is the question a benchmark that scores LLM extraction against an undeclared release [13, 29, 30] is currently unable to ask.
...
- transitive \texttt{revoked-by} closure, cycle-safe and bounded at ten hops;
...
\textbf{Table 12.} ATT\&CK version declarations in the documentation and configuration files of three deployed CTI corpora.
...
\textbf{Table 25.} Release declarations in a random sample of recent arXiv papers that use ATT\&CK technique identifiers. Wilson 95\% intervals.
...
That strategy is small enough to automate, and the package shows how without pretending the automation is a contribution. Because waves cannot be forecast, the honest form is not a detector but a standing re-measurement: a scheduled job fetches the newest bundle [1], rebuilds the release database, runs \texttt{attacknorm} over every artefact the organisation has pinned, which is what ATT\&CK Sync does for MITRE's own mappings and no consumer does for theirs [5], and fails when a ledger crosses a threshold the organisation chose, for instance more than two percent of identifiers dropped or any identifier changing abstraction level. The example workflow shipped under \texttt{code/examples/} is forty lines. What the measurements of this paper add to such a job is its calibration: the referential clock says what a dropped or demoted count means, the intensional clock says how often the thresholds deserve revisiting, and Section 6.3's finding that the version field is not a change signal says the job must diff descriptions itself rather than trust \texttt{x\_mitre\_version}, as MITRE's own change tool already does [34, 48]. A richer agent that also re-reads rewritten descriptions and re-labels affected artefacts is a systems problem and a separate paper; nothing here depends on it.
...
The paper practises the contract it proposes. Every measurement is computed from public artefacts by the scripts in \texttt{code/}, in order, from one shell script, with every stochastic component seeded; the inputs are the \texttt{attack-stix-data} repository at the commit recorded in the reproduction log, the four deployed corpora at their public repositories, and the CWE and CAPEC XML archives. The analysis release is Enterprise v19.2 and each experiment declares the source release it projects from. All computed results are shipped as JSON under \texttt{data/results/}, every table is generated from those files rather than transcribed, and a provenance check traces every number in the prose back to a result file. The residual-ledger tool of Section 8 is shipped as a stand-alone script with its own self-check, and the human-annotation instrument of Section 6.3 is shipped ready to run with its codebook, sample and answer key. The evidence corpus for the literature, one note per source with the fetch record and open-access provenance of each, is included. The package is intended for artifact evaluation at the Available and Reproduced levels.
...
14. ATT\&CK Design and Philosophy: inclusion criteria are…. https://attack.mitre.org/docs/ATTACK\_Design\_and\_Philosophy\_March\_2020.pdf
...
19. AVClass: alias resolution and label normalization as…. https://software.imdea.org/\textasciitilde{}juanca/papers/avclass\_raid16.pdf
...
30. *CTIBench (NeurIPS 2024): authoritative-source design…. https://proceedings.neurips.cc/paper\_files/paper/2024/file/5acd3c628aa1819fbf07c39ef73e7285-Paper-Datasets\_and\_Benchmarks\_Track.pdf
...
41. Griffioen, Booij \& Doerr — Quality Evaluation of Cyber…. https://link.springer.com/chapter/10.1007/978-3-030-57878-7\_14
42. IncreTTP (SecureComm 2025): incremental learning across…. https://link.springer.com/chapter/10.1007/978-3-032-23450-6\_2
...
48. MITRE diffStix changelog\_helper: ATT\&CK's own change…. https://github.com/mitre-attack/mitreattack-python/blob/master/mitreattack/diffStix/changelog\_helper.py
...
56. Ontology evolution vs ontology versioning: change…. https://www.researchgate.net/publication/37538421\_Change\_Management\_The\_Core\_Task\_of\_Ontology\_Versioning\_and\_Evolution
57. OWL versioning vocabulary: versionIRI, priorVersion…. https://www.w3.org/2007/OWL/wiki/Ontology\_Versions
